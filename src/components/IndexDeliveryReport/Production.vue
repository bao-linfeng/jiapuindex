 ...
 */
 const getDataList = async () => {
    const result = await index.productionYearOrgList({
        'orgKey': props.orgKey,
        'year': props.year,
        'page': page.value,
        'limit': limit.value,
    ...
    if(result.status == 200){
        tableData.value = result.result.list.map((ele, i) => {
            ele.i = (page.value - 1)*limit.value+(i+1);
...
const productionYearOrgDownload = async () => {
    const result = await index.productionYearOrgDownload({
        'orgKey': props.orgKey,
        'year': props.year,
    });
    if(result.status == 200){
...
const handleDownload = () => {
    ...
    tableData.value.forEach((ele) => {
        aoa.push([ele.i, ele.deliveryBatch, ele.gcKey, ele.genealogyName, ele.imageCount, ele.ADJImageCount, ele.JHNodeCount, ele.regularNodeCount, ele.totalPersonCount, ele.maleCount, ele.femaleCount, ele.bxCount, ele.allDeadO, ele.deadCount, ele.catalogStatistics, ele.deliveryMemo, ele.isRMSOrPipeline]);
    });
...
<template>
    <section class="production-wrap">
        <header class="header">
            ...
            <div class="right">
                <el-button type="primary" @click="productionYearOrgDownload">{{langData['下载']}}</el-button>
        ...
        <main class="main">
            ...
            <el-table 
                ...
                style="width: 100%">
                <el-table-column prop="i" :label="langData['序号']" width="100" align="center" />
